...
class DIRformer(nn.Module):
    def __init__(self, 
        ...
        self.ca4_3=ChannelAttention(int(dim*2**3)) # channel attention for feature fusion
        self.reduce_chan_level3 = nn.Conv2d(int(dim*2**3), int(dim*2**2), kernel_size=1, bias=bias)
        self.decoder_level3 = nn.Sequential(*[TransformerBlock(dim=int(dim*2**2), num_heads=heads[2], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[2])])


        self.up3_2 = Upsample(int(dim*2**2)) ## From Level 3 to Level 2
        self.ca3_2=ChannelAttention(int(dim*2**2)) # channel attention for feature fusion
        self.reduce_chan_level2 = nn.Conv2d(int(dim*2**2), int(dim*2**1), kernel_size=1, bias=bias)
        self.decoder_level2 = nn.Sequential(*[TransformerBlock(dim=int(dim*2**1), num_heads=heads[1], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[1])])
        
        self.up2_1 = Upsample(int(dim*2**1))  ## From Level 2 to Level 1  (NO 1x1 conv to reduce channels)
        self.ca2_1=ChannelAttention(int(dim*2**1)) # channel attention for feature fusion
        ...
        self.pixelshuffle_inp = nn.PixelShuffle(self.scale) # upsampler for inp
...
#RREM:reconstruction representation extraction module
class RREM(nn.Module):
    def __init__(self,n_feats = 64, n_encoder_res = 6,scale=4):
        super(RREM, self).__init__()
        self.scale=scale
        if scale == 2:
...
class IRR_DiffSR(nn.Module):
    def __init__(self,         
        ...
        linear_end= 0.99, 
        timesteps = 4 ):
        super(IRR_DiffSR, self).__init__()
        ...
        self.condition = RREM(n_feats=64, n_encoder_res=n_encoder_res,scale = scale)
  ...
  linear_end= linear_end, timesteps = timesteps)
    ...
    def forward(self, img, IPR=None):
        if self.training:
            IRR, pred_IRR_list=self.diffusion(img,IPR)
            sr = self.G(img, IRR)
            return sr, pred_IRR_list
        else:
            IRR=self.diffusion(img)
            sr = self.G(img, IRR)
            return sr
