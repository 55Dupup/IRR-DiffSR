...
def paired_paths_from_lmdb(folders, keys):
    ...
    ├── data.mdb
    ├── lock.mdb
    ├── meta_info.pkl or meta_info.txt

    The data.mdb and lock.mdb are standard lmdb files and you can refer to
    https://lmdb.readthedocs.io/en/release/ for more details.

    The meta_info.pkl or meta_info.txt is a specified file to record the meta information
    of our datasets. It will be automatically created when preparing
    datasets by our provided dataset tools.
    If it is a pkl file, it is a dictionary with two keys: "keys" and "resolution".
    The value of "keys" is a list of image names (with extension).
    The value of "resolution" is a list of image shapes.
    Example: {"keys": ["baboon.png", "barbara.png", ...], "resolution": [(120,125,3), (256,256,3), ...]}
    If it is a txt file, each line in the txt file records
    ...
    import os
    input_meta_info_path = osp.join(input_folder, 'meta_info')
    input_meta_info_ext = os.path.splitext(input_meta_info_path)[1]
    gt_meta_info_path = osp.join(gt_folder, 'meta_info')
    gt_meta_info_ext = os.path.splitext(gt_meta_info_path)[1]
    if input_meta_info_ext != gt_meta_info_ext:
        raise ValueError(
            f'Extensions of meta_info files in {input_key}_folder and {gt_key}_folder are different.')
    else:
        # use different ways to load the meta_info files according to the extension
        if input_meta_info_ext == '.pkl':
            print('ext_:pkl=======================')
            # use pickle to load the meta_info.pkl files
            import pickle
            with open(input_meta_info_path + '.pkl', 'rb') as fin:
                input_meta_info = pickle.load(fin)
                input_lmdb_keys = input_meta_info['keys']
                input_lmdb_sizes = input_meta_info['resolution']
            with open(gt_meta_info_path + '.pkl', 'rb') as fin:
                gt_meta_info = pickle.load(fin)
                gt_lmdb_keys = gt_meta_info['keys']
                gt_lmdb_sizes = gt_meta_info['resolution']
        elif input_meta_info_ext == '.txt':
            print('ext_:txt==================')
            # use open to load the meta_info.txt files
            with open(input_meta_info_path + '.txt') as fin:
                input_lmdb_keys = [line.split('.')[0] for line in fin]
            with open(gt_meta_info_path + '.txt') as fin:
                gt_lmdb_keys = [line.split('.')[0] for line in fin]
        else:
            raise ValueError(
                f'Unsupported extension of meta_info files: {input_meta_info_ext}')
...
#    with open(osp.join(folder, 'meta_info.txt')) as fin:
    with open(osp.join(folder, 'meta_info.pkl')) as fin:
