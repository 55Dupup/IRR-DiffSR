...
class DDPM(nn.Module):
    ...
    def predict_start_from_noise(self, x_t, t, noise):#从噪声中预测x_0
        return (
    ...
    def forward(self, img,x=None):
        ...
        if self.training:
            ...
            for i in reversed(range(0, self.num_timesteps)):
                IRR, predicted_noise = self.p_sample(IRR, torch.full((b,), i,  device=device, dtype=torch.long), c,
                                clip_denoised=self.clip_denoised)
                pred_IRR_list.append(IRR)
            return IRR,pred_IRR_list
        else:       
            ...
            IRR = x_noisy
            for i in reversed(range(0, self.num_timesteps)):
                IRR, _ = self.p_sample(IRR, torch.full((b,), i,  device=device, dtype=torch.long), c,
                                clip_denoised=self.clip_denoised)
            # print(f'img shape:{img.shape}---IRR shape:{IRR.shape} num_timesteps:{self.num_timesteps}')
            return IRR
